...
const meta: Meta<typeof ProcessBar> = {
  title: 'Components/Gantt/ProcessBar',
  ...
  render: () => ({
    ...
    setup() {
      ...
      watch(startDay, (v) => { if (v >= endDay.value) endDay.value = v + 1 })
      watch(endDay, (v) => { if (v <= startDay.value) startDay.value = v - 1 })
      ...
      return { dayZero, totalDays, startDate, endDate, title, projectCode, color, opacity, startDay, endDay }
    },
    template: `
      <div style="max-width:800px;margin:0 auto;font-family:sans-serif;">
        <div style="position:relative;width:100%;height:36px;background:#f0f0f0;border-radius:6px;margin-bottom:24px;">
          <ProcessBar :dayZero="dayZero" :totalDays="totalDays" :startDate="startDate" :endDate="endDate" :title="title" :projectCode="projectCode" :color="color" :opacity="opacity" />
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;background:#f8f9fa;padding:16px;border-radius:8px;border:1px solid #e8e8e8;">
          ...
          <label style="display:flex;flex-direction:column;gap:4px;font-size:13px;font-weight:600;color:#444;">
            Название процесса
            <input type="text" v-model="title" style="width:100%;padding:4px 8px;border:1px solid #ccc;border-radius:4px;">
          </label>

          <label style="display:flex;flex-direction:column;gap:4px;font-size:13px;font-weight:600;color:#444;">
            Код проекта
            <input type="text" v-model="projectCode" style="width:100%;padding:4px 8px;border:1px solid #ccc;border-radius:4px;">
          </label>
